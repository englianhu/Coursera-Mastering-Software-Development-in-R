...
register_google(key = "AIzaSyBi8p5ps2D3tsdNoMwgCsWIzqwUE1Pw9Fk")

# Create map and add hurricane Ike storm observation
map_ike100 <- get_map("Louisiana", zoom = 6, maptype = "toner-background") %>%
  ggmap(extent = "device") +
  geom_hurricane(data = storm_observation,scale_radii = 1, alpha = 0.7,
...
map_ike50 <- get_map("Louisiana", zoom = 6, maptype = "toner-background") %>%
  ggmap(extent = "device") +
  geom_hurricane(data = storm_observation, scale_radii = 0.5, alpha = 0.7,
                 aes(x = longitude, y = latitude, 
                     r_ne = ne, r_se = se, r_nw = nw, r_sw = sw,
                     fill = wind_speed, color = wind_speed)) + 
  scale_color_manual(name = "Wind speed (kts)", 
...
grid.arrange(map_ike100, map_ike50, 
             ncol = 2)
